...
function updateHandUI() {
  document.querySelectorAll("#cards button").forEach((b, i) => {
    ...
    b.disabled = cooldowns[c] > 0 || elixir < allCards[c].cost;
...
class Tower {
  ...
  draw() {
    ...
    if (this.destroying) ctx.fillStyle = "#ff0000";
    else ctx.fillStyle = this.team === "player" ? "#a855f7" : "#ef4444";
    ctx.fillText(this.king ? "👑" : "🏰", this.x - this.size/2, this.y + this.size/2);
    ctx.fillStyle = this.team === "player" ? "#a855f7" : "#ef4444";
    ctx.fillRect(this.x - this.size/2, this.y - this.size/2, (this.hp / this.maxHp) * this.size, 5);
    if (this.destroying) { this.size *= 0.9; if (this.size < 5) this.hp = 0; }
  }
}

/* ================= INPUT ================= */
function placeCard(cardName, x, y) {
  const c = allCards[cardName];
  if (elixir < c.cost) return;
  elixir -= c.cost;
  cooldowns[cardName] = 2;
  if (!c.spell) troops.push(new Troop(x, y, "player", c));
  rotateDeck(cardName);
  updateHandUI();
}
...
canvas.onclick = e => {
  ...
  placeCard(selectedCard, x, y);
  selectedCard = null;
  canvas.style.cursor = "default";
};

canvas.onmousemove = e => {
  const r = canvas.getBoundingClientRect(); dragPos.x = e.clientX - r.left; dragPos.y = e.clientY - r.top;
};
canvas.ontouchmove = e => {
  const r = canvas.getBoundingClientRect(); dragPos.x = e.touches[0].clientX - r.left; dragPos.y = e.touches[0].clientY - r.top;
};
...
function loop() {
  ...
  for (let i = towers.length - 1; i >= 0; i--) {
    if (towers[i].hp <= 0 && !towers[i].destroying) {
      towers[i].destroying = true;
      if (towers[i].king) {
        if (towers[i].team === "player") enemyCrowns++; else playerCrowns++;
        started = false;
      }
    } else if (towers[i].hp <= 0 && towers[i].destroying && towers[i].size < 5) {
      towers.splice(i,1);
    }
  }
  ...
  // Drag preview
  if (selectedCard && allCards[selectedCard].emoji) {
    ctx.font = "28px serif";
